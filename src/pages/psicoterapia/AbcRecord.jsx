...
const AbcRecord = () => {
    ...
    const [records, setRecords] = useState(() => {
        try {
            ...
            console.error("Erro ao ler dados (navegador bloqueando):", e);
            return [];
        }
    });
    ...
    useEffect(() => {
        try {
            ...
            console.error("Erro ao salvar dados (navegador bloqueando):", e);
        }
    }, [records]);
    ...
    const handleSubmit = (e) => {
        ...
        let newRecords;
        if (form.id) {
            newRecords = records.map(r => r.id === form.id ? form : r);
        } else {
            newRecords = [{ ...form, id: Date.now() }, ...records];
        }
    ...
    return (
        <div className="sub-page">
            <header className="page-header">
                <button className="icon-btn" onClick={() => { setIsAdding(false); setForm({ id: null, date: new Date().toISOString().slice(0, 16), antecedent: '', behavior: '', consequence: '' }); navigate(-1); }}>
                    <ArrowLeft size={24} />
                </button>
                <h1>{isAdding ? (form.id ? 'Editar Registro' : 'Novo Registro ABC') : 'Registro ABC'}</h1>
                {!isAdding && (
                    <button className="icon-btn-primary" onClick={() => { setForm({ id: null, date: new Date().toISOString().slice(0, 16), antecedent: '', behavior: '', consequence: '' }); setIsAdding(true); }}>
                        <Plus size={24} />
                    </button>
                )}
            </header>
                    ...
                    <div className="list-grid">
                        {records.length === 0 ? (
                            <div className="empty-state">
                                <p className="empty-text">Nenhum registro ABC encontrado.</p>
                                <p style={{ fontSize: 12, color: '#999', marginTop: 10, maxWidth: 300 }}>
                                    Seus dados ficam salvos no navegador. Verifique se cookies/dados locais estão permitidos.
                                </p>
                            </div>
                        ) : (
                            records.map(r => (
                                    ...
                                    {expandedIds.includes(r.id) && (
                                        <div style={{ marginTop: 12, width: '100%', fontSize: 14 }}>
                                            ...
                                            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 10 }}>
                                                <button className="icon-btn" onClick={() => editRecord(r)} title="Editar">
                                                    <Edit2 size={16} />
                                                </button>
                                                <button className="delete-mini" onClick={() => deleteRecord(r.id)} title="Excluir">
                                                    <Trash2 size={16} />
                                                </button>
                                            </div>
                                        </div>
